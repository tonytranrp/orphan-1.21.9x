...
void TestCommand::execute(const std::vector<std::string>& args)
{
    ...
    if (arg == "enforcedebug")
    {
        Orphan::Prefs->mEnforceDebugging = !Orphan::Prefs->mEnforceDebugging;
        PreferenceManager::save(Orphan::Prefs);
        ChatUtils::displayClientMessage("Enforce debugging set to {}", Orphan::Prefs->mEnforceDebugging ? "true" : "false");
