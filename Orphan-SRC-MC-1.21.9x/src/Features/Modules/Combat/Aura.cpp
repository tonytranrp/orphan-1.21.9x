...
void Aura::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    for (auto actor : actors)
    {
        ...
        if (mLimitAngle)
        {
            ...
            glm::vec3* plrpos = player->getPos();
            glm::vec3* targetposPtr = actor->getPos();
            glm::vec3 targetpos = *targetposPtr;
            targetpos.y -= .6f;

            // Target direction:
            glm::vec3 directionToTarget = glm::normalize(targetpos - *plrpos);

            // Set forward direction:
            glm::vec3 forward = glm::vec3(
                cos(glm::radians(playerRots.y + 90)) * cos(glm::radians(playerRots.x)),
                sin(glm::radians(playerRots.x)),
                sin(glm::radians(playerRots.y + 90)) * cos(glm::radians(playerRots.x))
            );

            // Horizontal:
            glm::vec2 forwardXZ = glm::normalize(glm::vec2(forward.x, forward.z));
            glm::vec2 targetXZ = glm::normalize(glm::vec2(directionToTarget.x, directionToTarget.z));
            float horizontalAngle = glm::degrees(acos(glm::clamp(glm::dot(forwardXZ, targetXZ), -1.0f, 1.0f)));

            // Vertical:
            float verticalAngle = glm::degrees(asin(directionToTarget.y * -1) - asin(forward.y));

            // Is looking at target:
            if (not (horizontalAngle <= (mHorizontalMax.mValue / 2) && std::abs(verticalAngle) <= (mVerticalMax.mValue / 2))) continue;
        }
        ...
        if (mAttackMode.mValue == AttackMode::Synced)
        {
