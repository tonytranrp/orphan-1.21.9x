...
void AutoQueue::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    // Check for the hotkey (Backslash) and execute QueueCommand if enabled
    if (mHotKey.mValue && Keyboard::mPressedKeys[VK_OEM_5]) {
        queueForGame();
        if (mShowInChat.mValue) {
            ChatUtils::displayClientMessage("§bHotkey pressed! Queuing for §6" + mLastGame + "§b.");
        }
    }
