...
public:
    ...
    BoolSetting mHotKey = BoolSetting("Packet Hotkey", "Pushing backslash causes the command to execute", false);

    AutoQueue() : ModuleBase("AutoQueue", "Automatically queues for games", ModuleCategory::Misc, 0, false){
        addSettings(&mMode, &mQueueDelay, &mShowInChat, &mQueueOnDeath, &mQueueOnGameEnd, &mHotKey);
