...
void SkinStealer::saveSkin(Actor* actor)
{
    ...
    const uint8_t* skinData = skin->impl->mObject.mSkinImage.mImageBytes.data();
    int width = skin->impl->mObject.mSkinImage.mWidth;
    int height = skin->impl->mObject.mSkinImage.mHeight;
    int bytes = width * height * 4;
    std::vector<uint8_t> pngData = convToPng(std::vector<uint8_t>(skinData, skinData + bytes), width, height);
    if (pngData.empty()) return;

    std::string path = FileUtils::getOrphanDir() + "Skins\\" + actor->getRawName() + "_skin.png";
    std::ofstream file(path, std::ios::binary);
    file.write((char*)pngData.data(), pngData.size());
    file.close();

    const uint8_t* capeData = skin->impl->mObject.mCapeImage.mImageBytes.data();
    int capeWidth = skin->impl->mObject.mCapeImage.mWidth;
    int capeHeight = skin->impl->mObject.mCapeImage.mHeight;
    ...
    if (capeBytes > 0) {
        ...
        if (!capePngData.empty()) {
            path = FileUtils::getOrphanDir() + "Skins\\" + actor->getRawName() + "_cape.png";
    ...
    ChatUtils::displayClientMessage("Saved skin for " + actor->getRawName() + " to your Orphan\\Skins folder! (copied to clipboard)");
    ImGui::SetClipboardText(std::string(FileUtils::getOrphanDir() + "Skins\\").c_str());

    std::string resourcePatch = skin->impl->mObject.mResourcePatch;
...
void SkinStealer::applySkin(Actor* actor, const std::vector<uint8_t>& skinData, const std::vector<uint8_t>& capeData, int capeWidth, int capeHeight, const std::string& resourcePatch)
{
    ...
    skinPacket->mSkin.impl->mObject.mId = currentSkin->impl->mObject.mId;
    skinPacket->mSkin.impl->mObject.mPlayFabId = currentSkin->impl->mObject.mPlayFabId;
    skinPacket->mSkin.impl->mObject.mFullId = currentSkin->impl->mObject.mFullId;
    ...
    skinPacket->mSkin.impl->mObject.mResourcePatch = std::string("{\"geometry\": {\"default\": \"") + (isSlim ? "geometry.humanoid.customSlim" : "geometry.humanoid.custom") + "\"}}";
    skinPacket->mSkin.impl->mObject.mDefaultGeometryName = isSlim ? "geometry.humanoid.customSlim" : "geometry.humanoid.custom";

    int width, height, depth;
    std::vector<unsigned char> skinImage = converToRGBA(skinData, width, height, depth);
    skinPacket->mSkin.impl->mObject.mSkinImage.mImageBytes = mce::Blob::fromVector(skinImage);
    skinPacket->mSkin.impl->mObject.mSkinImage.mWidth = width;
    skinPacket->mSkin.impl->mObject.mSkinImage.mHeight = height;
    skinPacket->mSkin.impl->mObject.mSkinImage.mDepth = 4;

    if (!capeData.empty()) {
        std::vector<unsigned char> capeImage = converToRGBA(capeData, capeWidth, capeHeight, depth);
        skinPacket->mSkin.impl->mObject.mCapeImage.mImageBytes = mce::Blob::fromVector(capeImage);
        skinPacket->mSkin.impl->mObject.mCapeImage.mWidth = capeWidth;
        skinPacket->mSkin.impl->mObject.mCapeImage.mHeight = capeHeight;
        skinPacket->mSkin.impl->mObject.mCapeImage.mDepth = 4;
    }
