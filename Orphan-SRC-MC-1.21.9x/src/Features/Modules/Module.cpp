...
nlohmann::json Module::serialize()
{
    ...
    for (const auto setting : mSettings)
    {
        ...
        else if (setting->mType == SettingType::Keybind)
        {
            auto* keybindSetting = reinterpret_cast<KeybindSetting*>(setting);

            j["settings"].push_back(keybindSetting->serialize());
        }
        else if (setting->mType == SettingType::Divider)
        {
            auto* dividerSetting = reinterpret_cast<DividerSetting*>(setting);

            j["settings"].push_back(dividerSetting->serialize());
        }
    }
    return j;
}
...
Setting* Module::getSetting(const std::string& string)
{
    ...
    std::string normInput = normalize(string);
    for (auto setting : mSettings)
    {
        if (normalize(setting->mName) == normInput)
        {
            return setting;
        }
    }
