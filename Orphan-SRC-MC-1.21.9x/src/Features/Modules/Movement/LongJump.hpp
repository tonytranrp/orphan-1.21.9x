...
public:
    NumberSetting mSpeed = NumberSetting("Speed", "How fast you go", 1, 0, 40, 0.1);
    NumberSetting mHeight = NumberSetting("Height", "How high you go", 0.42, 0, 10, 0.1);
    BoolSetting mTimerBoost = BoolSetting("Timer Boost", "Boosts timer when jumping", false);
    NumberSetting mTimer = NumberSetting("Timer", "The timer to set when this module is enabled", 28.30, 0.1, 60, 0.1);
