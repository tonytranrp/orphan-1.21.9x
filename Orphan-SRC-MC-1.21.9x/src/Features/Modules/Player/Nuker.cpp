...
bool Nuker::isValidBlock(glm::ivec3 blockPos) {
    ...
    if (mBlockType.mValue == BlockType::Bed) {
        return block->getmLegacy()->getmName() == "bed_hitbox";
    }

    if (mBlockType.mValue == BlockType::Specified && block->getmLegacy()->getmName() != specifiedBlockID) {
        return false;
    }
