...
void Scaffold::onDisable()
{
    ...
    glm::vec3 Posi = { pPos->x, mStartY + 2.62f, pPos->z };
    ...
    if (mIsTowering)
    {
        ...
        {
            player->getStateVectorComponent()->mVelocity.y = -3.0f;
        }
    }
}
...
void Scaffold::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    float places = mPlaces.as<float>();

    // LowPlaces: set places per tick to 0.55
    if (mBypassMode.mValue == BypassMode::LowPlaces) {
        places = 0.55f;
    }

    if (mFastClutch.mValue && player->getFallDistance() > mClutchFallDistance.mValue)
    {
        places = mCluchPlaces.as<float>();
    }

    int wholePlaces = static_cast<int>(places);
    float fractionalPart = places - wholePlaces;

    for (int i = 0; i < wholePlaces; i++)
    {
    ...
    if (fractionalPart > 0.0f)
    {
        ...
        uint64_t timeBetweenPlacements = static_cast<uint64_t>((1.0f / fractionalPart) * 50.0f);

        if (currentTime - mLastPlaceTime >= timeBetweenPlacements)
        {
            if (tickPlace(event))
            {
                mLastPlaceTime = currentTime;
            }
        }
    }
}
...
bool Scaffold::tickPlace(BaseTickEvent& event)
{
    ...
    if (mLockY.mValue && player->isOnGround() && !mIsTowering) {
        float groundY = player->getPos()->y - 2.62f;
        if (abs(groundY - mStartY) > 0.1f) {
            mStartY = groundY;
        }
    }
    ...
    bool isStrafing = Keyboard::isStrafing();
