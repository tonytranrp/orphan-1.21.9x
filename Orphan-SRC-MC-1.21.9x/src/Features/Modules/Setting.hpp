...
public:
    ...
    float sliderAnimValue = 0.0f;
    float lastValue = 0.0f;
    int mKey = 0;

    float visibilityAnim = 1.0f;
    bool hasVisibilityAnim = false;
...
public:
    ...
    NumberSetting(std::string name, std::string description, float value, float min, float max, float step)
        : Setting(std::move(name), std::move(description), SettingType::Number), 
          ...
          lastValue(value), sliderAnimValue(value), sliderProgress((value - min) / (max - min))
    {
    }
    ...
    void setValue(float value)
    {
        ...
        sliderAnimValue = mValue;
        sliderProgress = (mValue - mMin) / (mMax - mMin);
    }
...
class DividerSetting : public Setting
{
public:
	bool mDivider = true;
    DividerSetting(std::string name, std::string description)
        : Setting("", std::move(description), SettingType::Divider)
    {
        ...
        for (size_t i = 0; i < name.length(); i++) {
            ...
            if (name[i] == ' ' && i > 0 && i < name.length() - 1 && 
                name[i-1] == '-' && name[i+1] == '-') {
                
                continue;
            }
            processedText += name[i];
        }
        mName = processedText;
    }
    ...
    void render()
    {
        ...
        ImGui::Separator();
        if (!mDescription.empty() && ImGui::IsItemHovered())
        {
...
// KeybindSetting: Allows binding a key to a feature/module.
