...
{
    BlockESP* blockESP = static_cast<BlockESP*>(gFeatureManager->mModuleManager->getModule("BlockESP"));
    if (!blockESP) {
        spdlog::error("OreESP module not found!");
        return;
    }

    gFeatureManager->mDispatcher->listen<RenderEvent, &BlockESP::onRenderEvent>(blockESP);
    gFeatureManager->mDispatcher->listen<BaseTickEvent, &BlockESP::onBaseTickEvent>(blockESP);
    gFeatureManager->mDispatcher->listen<BlockChangedEvent, &BlockESP::onBlockChangedEvent>(blockESP);
    gFeatureManager->mDispatcher->listen<PacketInEvent, &BlockESP::onPacketInEvent>(blockESP);

    blockESP->mChestESP = true;
}
    ...
    for (uint16_t x = 0; x < 16; x++)
    {
        for (uint16_t z = 0; z < 16; z++)
        {
            for (uint16_t y = 0; y < (blockSource->getBuildHeight() - blockSource->getBuildDepth()) / chunk->getSubChunks()->size(); y++)
            {
                ...
                bool ischest = blockId == CHEST || blockId == ENDER_CHEST || blockId == TRAPPED_CHEST;
                if (blockId == 0 || ((mChestESP == true && BlockESPEnabled == false) && !ischest))
                {
                    mFoundBlocks.erase(BlockPos((processChunk.x * 16) + x, y + (subChunk.subchunkIndex * 16), (processChunk.y * 16) + z));
                    continue;
                }
                if (std::ranges::find(enabledBlocks, blockId) == enabledBlocks.end()) continue;
                ...
                // Only add new blocks with discovery time and player position
                if (mFoundBlocks.find(pos) == mFoundBlocks.end()) {
                    mFoundBlocks[pos] = {
                        found,
                        AABB(pos, glm::vec3(1.f, 1.f, 1.f)),
                        getColorFromId(blockId),
                        static_cast<uint64_t>(NOW),  // Current time as discovery time
...
void BlockESP::reset()
{
    ...
    ClientInstance* ci = ClientInstance::get();
    Actor* player = ci->getLocalPlayer();
    blockESP->mSearchStart = NOW;
    blockESP->mFoundBlocks.clear();
    blockESP->mStepsCount = 0;
    blockESP->mSteps = 1;
    blockESP->mDirectionIndex = 0;
    blockESP->mSubChunkIndex = 0;
    if (!player) return;
    BlockSource* blockSource = ci->getBlockSource();

    blockESP->mSearchCenter = ChunkPos(*player->getPos());
    blockESP->mCurrentChunkPos = blockESP->mSearchCenter;
}
...
void BlockESP::onDisable()
{
    ...
    // Reset opacity values by clearing found blocks and resetting search start time
...
{
    ChestESP* chestESP = static_cast<ChestESP*>(gFeatureManager->mModuleManager->getModule("ChestESP"));
    if (!chestESP) {
        spdlog::error("ChestESP module not found!");
        return;
    }
    if (!ClientInstance::get()->getLevelRenderer()) {
        reset();
    ...
    auto plrpos = *ClientInstance::get()->getLocalPlayer()->getPos();
    float currange = dabl.getDistance(plrpos);
    auto blockId = event.mNewBlock->mLegacy->getBlockId();
    bool ischest = blockId == CHEST || blockId == ENDER_CHEST || blockId == TRAPPED_CHEST;
    if (currange > mRadius.mValue && !ischest) return;
    if ((currange > chestESP->mChestRadius.mValue && chestESP->mChestRadius.mValue != 100) && ischest) return;
    ...
    if (isValidBlock(blockId) && std::ranges::find(enabledBlocks, blockId) != enabledBlocks.end())
    {
        const Block* block = event.mNewBlock;
        mFoundBlocks[event.mBlockPos] = {
            block,
            AABB(event.mBlockPos, glm::vec3(1.f, 1.f, 1.f)),
            getColorFromId(block->mLegacy->getBlockId()),
            static_cast<uint64_t>(NOW),  // Current time as discovery time
            plrpos // Current player position
        };
...
void BlockESP::onBaseTickEvent(BaseTickEvent& event)
{
    ...
    int64_t freq = static_cast<int64_t>(mUpdateFrequency.mValue * 50.f);
    int64_t now = NOW;
...
void BlockESP::onRenderEvent(RenderEvent& event)
{
    ...
    float fadeDurationMs = 0.1f * 1000.0f;

    for (auto& [pos, block] : mFoundBlocks)
    {
        float blockmag = distance(glm::vec3(pos), playerPos);
        auto blockId = block.block->mLegacy->getBlockId();
        bool ischest = blockId == CHEST || blockId == ENDER_CHEST || blockId == TRAPPED_CHEST;
        if (BlockESPEnabled == false && !ischest) continue;
        if (mChestESP == false && ischest) continue;
        if (blockmag > mRadius.mValue && !ischest) continue;
        if ((blockmag > chestESP->mChestRadius.mValue && chestESP->mChestRadius.mValue != 100) && ischest) continue;
        if (pos.y < (playerPos.y - mBottomY.mValue) && !ischest) continue;
        if (std::ranges::find(enabled, blockId) == enabled.end()) continue;

        float timeSinceDiscovery = static_cast<float>(currentTime - block.discoveryTime);
        float timeFade = std::min(timeSinceDiscovery / fadeDurationMs, 1.0f);

        float maxDistance = ischest ? chestESP->mChestRadius.mValue : mRadius.mValue;
        float fadeRange = ischest ? chestESP->mChestRadius.mValue : mFadeRange.mValue;
        float fullRenderDistance = ischest ? chestESP->mChestRadius.mValue : mFullRenderDistance.mValue;
        float distanceFade = 1.0f;

        if (mEnableFade.mValue && blockmag > fullRenderDistance) {
            if (blockmag <= fadeRange) {
                distanceFade = 1.0f - ((blockmag - fullRenderDistance) / (fadeRange - fullRenderDistance));
            }
            else {
                float baseFade = 1.0f - ((blockmag - fadeRange) / (maxDistance - fadeRange));
                float fadeAdjustment = 1.0f - ((fadeRange - fullRenderDistance) / (fadeRange - fullRenderDistance));
                distanceFade = baseFade * fadeAdjustment;
            }
            distanceFade = std::clamp(distanceFade, 0.0f, 1.0f);
        }

        glm::vec3 discoveryDir = glm::normalize(playerPos - block.playerPosAtDiscovery);
        glm::vec3 blockDir = glm::normalize(glm::vec3(pos) - block.playerPosAtDiscovery);
        float directionFactor = glm::dot(discoveryDir, blockDir) * 0.5f + 0.5f;

        float alpha = timeFade * distanceFade * directionFactor;

        ImColor color = block.color;
        color.Value.w = alpha;
        ImColor fillColor = color;
        fillColor.Value.w = alpha * mOpacity.mValue;
        ...
        if (mRenderMode.mValue == BlockRenderMode::Both || mRenderMode.mValue == BlockRenderMode::Filled) {
            drawList->AddConvexPolyFilled(imPoints.data(), imPoints.size(), fillColor);
        }
    }
}