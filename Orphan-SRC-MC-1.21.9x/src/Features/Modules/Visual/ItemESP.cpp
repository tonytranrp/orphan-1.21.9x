...
void ItemESP::onRenderEvent(RenderEvent& event)
{
    ...
    for (auto actor : items)
    {
        ...
        //ChatUtils::displayClientMessage(fmt::format("Origin: ({}, {}, {})", origin.x, origin.y, origin.z));
        //ChatUtils::displayClientMessage(fmt::format("Pos: ({}, {}, {})", pos.x, pos.y, pos.z));
        //ChatUtils::displayClientMessage(fmt::format("Screen: ({}, {})", screen.x, screen.y));
        //ChatUtils::displayClientMessage(fmt::format("Resolution: ({}, {})", ci->getGuiData()->mResolution.x, ci->getGuiData()->mResolution.y));
