...
void UpdateForm::getCommits()
{
    ...
    /*HttpRequest request(HttpMethod::GET, OAuthUtils::sEndpoint + "getCommitsBetween?startHash=" + latestHash + "&endHash=" + ORPHAN_BUILD_VERSION, "", "", &UpdateForm::onHttpResponse, nullptr);
    request.sendAsync();*/
    mRequest.mMethod = HttpMethod::GET;
    //mRequest.mUrl = OAuthUtils::sEndpoint + "getCommitsBetween?startHash=" + latestHash + "&endHash=" + ORPHAN_BUILD_VERSION;
...
void UpdateForm::onRenderEvent(RenderEvent& event)
{
    ...
    float titleHeight = ImGui::GetFont()->CalcTextSizeA(30, FLT_MAX, 0, "New Orphan update installed!").y;
    ...
    if (longestCommit < ImGui::GetFont()->CalcTextSizeA(30, FLT_MAX, 0, "New Orphan update installed!").x)
        longestCommit = ImGui::GetFont()->CalcTextSizeA(30, FLT_MAX, 0, "New Orphan update installed!").x;
    ...
    ImVec2 titleSize = ImGui::GetFont()->CalcTextSizeA(30, FLT_MAX, 0, "New Orphan update installed!");
    titlePos.y += (titleHeight - titleSize.y) / 2;
    titlePos.x = renderPos.x + (renderSize.x - titleSize.x) / 2;
    drawlist->AddText(ImGui::GetFont(), 30, titlePos, IM_COL32(255, 255, 255, 255), "New Orphan update installed!");
