...
void KeyHook::onKey(uint32_t key, bool isDown)
{
    ...
    for (auto& module : gFeatureManager->mModuleManager->getModules())
    {
        ...
        if (module->mKey == key || module->mKeybindSetting.mKey == key)
        {
        ...
        // KeybindSetting support: toggle module if any KeybindSetting in mSettings matches the pressed key (but not if it's the main key)
        /*
        if (isDown) {
            for (Setting* setting : module->mSettings) {
                auto keybindSetting = dynamic_cast<KeybindSetting*>(setting);
                if (keybindSetting && keybindSetting->mKey == key && key != module->mKey) {
                    module->toggle();
                }
            }
        }
        // Disabled due to crash -- needs further investigation
        */
    }
}
