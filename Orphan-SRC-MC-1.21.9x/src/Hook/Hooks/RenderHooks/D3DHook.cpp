...
HRESULT D3DHook::present(IDXGISwapChain3* swapChain, UINT syncInterval, UINT flags)
{
    ...
    if (!once)
    {
        ...
        if (swapChain->GetDevice(__uuidof(ID3D12Device), reinterpret_cast<void **>(&gDevice12)) == S_OK) {
            ...
            if (Orphan::Prefs->mFallbackToD3D11)
            {
...
void D3DHook::initImGui(ID3D11Device* device, ID3D11DeviceContext* deviceContext)
{
    ...
    if (!onc)
    {
        ...
        FileUtils::writeResourceToFile(res, FileUtils::getOrphanDir() + "BlinkerSkins\\README.txt");
...
void D3DHook::s_init()
{
    Orphan::console->info("Initializing D3DHook");
    // Attempt to init on D3D12
    if (kiero::init(kiero::RenderType::D3D12) == kiero::Status::Success)
    {
        Orphan::console->info("Initialized kiero [D3D12]");
    ...
    {
        Orphan::console->info("Initialized kiero [D3D11]");
    ...
    Orphan::console->error("Failed to initialize kiero");
    MessageBoxA(NULL, "Failed to initialize kiero", "Orphan", MB_OK | MB_ICONERROR);
}
...
{
    Orphan::console->info("Shutting down D3DHook");
