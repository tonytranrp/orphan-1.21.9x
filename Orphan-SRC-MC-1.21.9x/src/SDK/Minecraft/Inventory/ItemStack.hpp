...
public:
    ...
    std::string getCustomName() {
        std::string str;
        MemUtils::callFastcall<void, ItemStack*, std::string*>(SigManager::ItemStack_getCustomName, this, &str);
        return str;
    }

    std::string getCustomNameUnformatted() {
        std::string str;
        MemUtils::callFastcall<void, ItemStack*, std::string*>(SigManager::ItemStack_getCustomName, this, &str);
        std::regex colourcodes("[^a-zA-Z0-9 ][a-zA-Z0-9]");
        str = std::regex_replace(str, colourcodes, "");
        std::regex cleanupsymbols("[^a-zA-Z0-9 ]");
        str = std::regex_replace(str, cleanupsymbols, "");
        return str;
    }
