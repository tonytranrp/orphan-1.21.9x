...
namespace mce {
    ...
    public:
        ...
        [[nodiscard]] constexpr Blob(std::span<value_type> s, Deleter deleter = {}) : mSize(s.size()) { // NOLINT
        ...
        [[nodiscard]] constexpr std::span<value_type> view() const { return { data(), size() }; }
    ...
    public:
        ...
        ResourceLocation(bool external, std::string filePath) {
            ...
            if (external)
                this->mFileSystem = 0x0002; // ResourceFileSystem::Raw
            else this->mFileSystem = 0x0000; // ResourceFileSystem::UserPackage
            _computeHashes();
        };
    ...
    class TexturePtr {
    public:
        std::shared_ptr<struct BedrockTextureData> mClientTexture;
        std::shared_ptr<ResourceLocation> mResourceLocation;

        TexturePtr() = default;
        [[nodiscard]] std::string getFilePath() const {
            return mResourceLocation ? mResourceLocation->mFilePath : std::string("");
        }
    };
}
