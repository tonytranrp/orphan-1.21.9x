...
public:
	...
	//DEFINE_INDEX_FIELD(ClientInstance_getScreenName, "48 8B 80 B0 08 00 00 FF 15 ?? ?? ?? ?? 90", 3, OffsetType::Index);
	...
	DEFINE_INDEX_FIELD(ClientInstance_getOptions_mGfxGamma, "48 8b 80 ? ? ? ? ff 15 ? ? ? ? 84 c0 0f 84 ? ? ? ? 48 8b 07 48 8b cf 48 8b 80 ? ? ? ? ff 15 ? ? ? ? 84 c0 74 ? 49 8b 46", 3, OffsetType::Index);
	...
	DEFINE_INDEX_FIELD(Level_getLevelData, "48 8b 80 ? ? ? ? ff 15 ? ? ? ? 33 ff 48 8d 0d", 3, OffsetType::Index); // Updated to 1.21.90
	...
	DEFINE_INDEX_FIELD_TYPED(uint8_t, ContainerManagerModel_getContainerType, "48 8b 03 48 8b cb 48 8b 40 ? ff 15 ? ? ? ? 84 c0 74 ? 80 7b", 3, OffsetType::Index); // Updated to 1.21.90
	...
	DEFINE_INDEX_FIELD_TYPED(uint8_t, Container_getItem, "48 8b 40 ? ff 15 ? ? ? ? 48 8b d8 eb ? 48 8d 1d ? ? ? ? 40 38 7b", 3, OffsetType::Index);
	...
	DEFINE_FIELD(ClientInstance_getScreenName, 278); // i will make sig for it later (maybe)
	DEFINE_FIELD(ClientInstance_mFovX, 0x740); // 1.21.80
	DEFINE_FIELD(ClientInstance_mFovY, 0x754); // 1.21.80
	DEFINE_FIELD(ClientInstance_mGuiData, 0x5B8); // i will make sig for it later (maybe)
