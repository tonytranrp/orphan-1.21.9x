...
void SigManager::initialize()
{
    ...
    for (const auto& sig : mSigs) {
        if (sig.second != 0) Orphan::console->info("[signatures] found {} @ {}", sig.first, MemUtils::getMbMemoryString(sig.second));
    }

    for (const auto& sig : mSigs) {
        if (sig.second == 0) Orphan::console->critical("[signatures] failed to find {}", sig.first);
    ...
    Orphan::console->info("[signatures] initialized in {}ms, {} total sigs scanned", diff, mSigScanCount);
    mIsInitialized = true;
}
