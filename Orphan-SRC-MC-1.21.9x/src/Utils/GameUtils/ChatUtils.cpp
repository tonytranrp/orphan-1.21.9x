...
void ChatUtils::displayClientMessage(const std::string& msg)
{
    ...
    {// FUCK YOU DONT CHANGE THE PREIX COLOR U NIGGER
        ClientInstance::get()->getGuiData()->displayClientMessageQueued("§aorphan§7 » §r" + msg);
        return;
    }

    // Newline handling
    std::string formattedMsg = "§aorphan§7 » §r";
...
{
    ClientInstance::get()->getGuiData()->displayClientMessageQueued("§aorphan§7 » §7[" + subcaption + "§7] §r" + msg);
}
