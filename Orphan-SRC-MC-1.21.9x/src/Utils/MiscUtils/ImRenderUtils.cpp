...
// Static cache variables for blur optimization
static ImVec4 lastBlurRect = ImVec4(0, 0, 0, 0);
static float lastBlurStrength = 0.0f;
static int lastBlurPasses = 0;
static bool blurNeedsUpdate = true;
...
void ImRenderUtils::addBlur(const ImVec4& pos, float strength, float radius, ImDrawList* list, bool optimized)
{
	...
	D2D::addBlurOptimized(list, strength, pos, radius);
    
}

void ImRenderUtils::drawText(ImVec2 pos, const std::string& textStr, const ImColor& color, float textSize, float alpha, bool shadow, int index, ImDrawList* d)
{
    if (!ImGui::GetCurrentContext())
        return;
...
void ImRenderUtils::fillShadowRectangle(ImVec4 pos, const ImColor& color, float alpha, float thickness, ImDrawFlags flags, float radius, ImDrawList* list)
{
    if (!ImGui::GetCurrentContext())
        return;
...
void ImRenderUtils::fillShadowCircle(ImVec2 pos, float radius, const ImColor& color, float alpha, float thickness, ImDrawFlags flags, float segments)
{
    if (!ImGui::GetCurrentContext())
        return;
...
void ImRenderUtils::drawRoundRect(ImVec4 pos, const ImDrawFlags& flags, float radius, const ImColor& color, float alpha, float lineWidth)
{
    if (!ImGui::GetCurrentContext())
        return;
...
void ImRenderUtils::fillGradientOpaqueRectangle(ImVec4 pos, const ImColor& firstColor, const ImColor& secondColor, float firstAlpha, float secondAlpha)
{
    if (!ImGui::GetCurrentContext())
        return;
...
void ImRenderUtils::fillRoundedGradientRectangle(ImVec4 pos, const ImColor& firstColor, const ImColor& secondColor, float radius, float firstAlpha, float secondAlpha, ImDrawFlags flags)
{
	if (!ImGui::GetCurrentContext())
		return;
...
void ImRenderUtils::applyBlur(const ImVec4& rect, float strength, int passes) {
    if (strength <= 0.0f) return;

    static float lastTime = ImGui::GetTime();
    static float currentTime = ImGui::GetTime();
    currentTime = ImGui::GetTime();

    // Force update if significant time has passed
    if (currentTime - lastTime > 1.0f) {
        blurNeedsUpdate = true;
        lastTime = currentTime;
    }

    addBlur(rect, strength, 0.0f, ImGui::GetBackgroundDrawList(), true);

    lastBlurRect = rect;
    lastBlurStrength = strength;
    lastBlurPasses = passes;

    ImGui::GetBackgroundDrawList()->AddRectFilled(
        ImVec2(rect.x, rect.y),
        ImVec2(rect.z, rect.w),
        ImGui::GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.1f))
    );
}

void ImRenderUtils::invalidateBlurCache() {
    blurNeedsUpdate = true;
}

void ImRenderUtils::fillTransparentRectangle(ImVec4 pos, const ImColor& color, float alpha, float radius, ImDrawList* list, ImDrawFlags flags) {
    if (!ImGui::GetCurrentContext())
        return;

    // Apply blur first for transparency effect
    addBlur(pos, 5.0f, 0.0f, list, true);
    
    // Create a very transparent overlay
    ImColor overlayColor = color;
    overlayColor.Value.w = alpha * 0.05f; // Extremely low alpha
...
void ImRenderUtils::fillTransparentGradient(ImVec4 pos, const ImColor& firstColor, const ImColor& secondColor, float radius, float alpha, ImDrawFlags flags) {
    if (!ImGui::GetCurrentContext())
        return;

    // Apply stronger blur first for better transparency effect
    addBlur(pos, 8.0f, 0.0f, ImGui::GetBackgroundDrawList(), true);
    
    // Create darker gradient colors
    ImColor gradStart = firstColor;
    ImColor gradEnd = secondColor;
    
    // Darken the colors
    gradStart.Value.x *= 0.3f;
    gradStart.Value.y *= 0.3f;
    gradStart.Value.z *= 0.3f;
    gradStart.Value.w = alpha * 0.15f; // Less transparent (was 0.05f)
    
    gradEnd.Value.x *= 0.3f;
    gradEnd.Value.y *= 0.3f;
    gradEnd.Value.z *= 0.3f;
    gradEnd.Value.w = alpha * 0.15f; // Less transparent (was 0.05f)
    
    // Add a very subtle dark background first
    ImGui::GetBackgroundDrawList()->AddRectFilled(
        ImVec2(pos.x, pos.y),
        ImVec2(pos.z, pos.w),
        ImColor(0, 0, 0, int(20 * alpha)),
        radius,
        flags
    );
    
    // Then add the gradient
    fillRoundedGradientRectangle(
        pos,
        gradStart,
        gradEnd,
        radius,
