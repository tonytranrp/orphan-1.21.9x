...
public:
	...
	static void applyBlur(const ImVec4& rect, float strength, int passes = 2);
	static void invalidateBlurCache();
	static void fillTransparentRectangle(ImVec4 pos, const ImColor& color, float alpha, float radius = 0.f, ImDrawList* list = ImGui::GetBackgroundDrawList(), ImDrawFlags flags = 0);
	static void fillTransparentGradient(ImVec4 pos, const ImColor& firstColor, const ImColor& secondColor, float radius, float alpha, ImDrawFlags flags = 0);
};
