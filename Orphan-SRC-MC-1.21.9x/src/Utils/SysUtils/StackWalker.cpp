...
void StackWalker::LoadModuleSymbols(const std::vector<std::wstring>& modulePaths) {
    HMODULE mainModule = Orphan::mModule;
    ...
    if (!modulePathStr.empty()) {
        ...
        DWORD64 result = SymLoadModuleEx(GetCurrentProcess(), Orphan::mModule, modulePathStr.c_str(), nullptr, 0, 0,
                                         nullptr, 0);

        if (result == 0) {
            spdlog::error("Failed to load symbols for the main module.");
        } else {
            spdlog::info("Loaded symbols for Orphan.dll.");
        }
